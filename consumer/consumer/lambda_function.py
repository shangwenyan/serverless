...
def apply_sentiment(df, column="reviews"):
...
def write_s3(df, bucket, review):
    """Write S3 Bucket"""

    csv_buffer = StringIO()
    df.to_csv(csv_buffer)
    s3_resource = boto3.resource('s3')
    filename = f"{review}_sentiment.csv"
...
def lambda_handler(event, context):
    ...
    LOG.info(f"Creating dataframe with values: {reviews}")
    df = names_to_wikipedia(reviews)
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentiment", review=reviews)